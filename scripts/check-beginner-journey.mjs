...
assert.equal(await page.evaluate(() => document.activeElement?.textContent?.includes('Agora não')), true, 'Tab deve avançar dentro do guia');
await page.keyboard.press('Tab');
assert.equal(await page.evaluate(() => document.activeElement?.textContent?.includes('Entendi, vamos começar')), true, 'Tab no último controle deve voltar ao primeiro');
await page.keyboard.press('Shift+Tab');
assert.equal(await page.evaluate(() => document.activeElement?.textContent?.includes('Agora não')), true, 'Shift+Tab no primeiro controle deve voltar ao último');
