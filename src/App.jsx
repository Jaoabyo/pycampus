...
import { Icon, Progress, ExplicaEtapa } from './ui.jsx';
import { modules, lessons, projects, resources } from './curriculum.js';
import { stepsFor, buildDone } from './project-steps.js';
import { lessonAllowed, lessonIsOpen, explicaTrabalho, blockingSummary, missingSummary, moduleIsOpen, moduleIndexForLesson, moduleRequirements, pendingStageWork, practiceIsOpen, projectIsOpen } from './progression.js';
...
// e 1 projeto" — e o estudante ficava sem ter para onde ir, embora o botão da Visão geral
...
function LessonView({ lesson, state, update, notify, openLesson, openProject, navigate }) {
  ...
  useEffect(() => {
    if (ready && !previousReady.current) setReadyNotice(true);
  ...
  return <>{readyNotice && ready && <ReadyToComplete onComplete={finish} onClose={() => setReadyNotice(false)} />}
    ...
    <div className="page-heading lesson-heading">
      ...
      <span className="lesson-count">Aula {position + 1} de {lessons.length}</span>
    </div>
    <div className="lesson-steps tab-row" aria-label="Passos desta aula">{steps.map(([number, label, icon, done]) => <button key={number} onClick={() => goToStep(number)}><Icon name={done ? 'CheckCircle2' : icon} size={14} className={done ? 'step-check' : ''} />{number} · {label}</button>)}</div>
    <div className="lesson-layout">
      <div>
        <LessonOrientation lesson={lesson} lessons={lessons} completed={state.completed} openLesson={openLesson} />
        <section className="card theory" id="passo-1">
          <StepHead number={1} title="Entenda a ideia" icon="Lightbulb" color="purple" />
          <SimpleConcept lesson={lesson} />
        </section>
        <section className="card theory" id="passo-2">
        ...
        {lesson.id === 'condicoes' && <DecisionLab />}
      </div>
      <div>
        <section className="card lesson-work" id="passo-3">
        ...
        <section className="card review" id="passo-4">
...
// Calculadora de orçamento" e "Seu próximo passo: Decisões com if, elif e else". O herói lia
// pendingStageWork, que é o que de fato destrava a etapa; o cartão lia nextOpenLesson, a
// primeira aula não concluída, ignorando os miniprojetos, as pontes e o projeto que faltavam.
...
export default function App() {
  ...
  const [page, setPage] = useState('dashboard'), [selectedLesson, setSelectedLesson] = useState('ola'), [selectedProject, setSelectedProject] = useState('calculadora'), [mobileOpen, setMobileOpen] = useState(false), [modal, setModal] = useState(null), [toast, setToast] = useState(''), [query, setQuery] = useState(''), [filter, setFilter] = useState('Todas as etapas'), [expanded, setExpanded] = useState('fundamentos');
  ...
  const navigate = target => { setPage(target); setMobileOpen(false); window.scrollTo({ top: 0, behavior: 'instant' }); };
  const openLesson = id => {
