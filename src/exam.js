...
export function podeFazerProva(state) {
  return (state?.completed?.length || 0) >= MINIMO_DE_AULAS;
}

// Sorteia entre as aulas que o estudante realmente concluiu: cobrar o que não foi estudado
// não mede recall, mede sorte.
export function montarProva(state, quantidade = QUESTOES_PADRAO, aleatorio = Math.random) {
  const feitas = lessons.filter(lesson => state?.completed?.includes(lesson.id));
  const sorteadas = [...feitas];
  for (let i = sorteadas.length - 1; i > 0; i--) {
    const j = Math.floor(aleatorio() * (i + 1));
    [sorteadas[i], sorteadas[j]] = [sorteadas[j], sorteadas[i]];
  }
  return sorteadas.slice(0, Math.min(quantidade, sorteadas.length)).map(lesson => ({
    id: lesson.id,
    titulo: lesson.title,
...
export function normalizeProvas(input) {
  ...
  const data = valor => typeof valor === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(valor);
  return (Array.isArray(input?.provas) ? input.provas : [])
    .filter(prova => prova && data(prova.data) && Number.isInteger(prova.total) && prova.total > 0 && prova.total <= 48)
    .slice(-20)
    .map(prova => ({
      data: prova.data,
      total: prova.total,
      ...
      falhas: (Array.isArray(prova.falhas) ? prova.falhas : []).filter(id => ids.has(id)).slice(0, 48)
    }));
}
